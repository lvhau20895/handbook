...
const login = async (req, res) => {
	...
	try {
		...
		if (!user) return failCode(res, "user do not exists");
		const checkPassword = bcrypt.compareSync(password, user.password);
		if (!checkPassword) return failCode(res, "password not found");
...
const getUser = async (req, res) => {
	...
	const { data, bearer } = req.user;
	res.status(200).json({ data, bearer });
};
